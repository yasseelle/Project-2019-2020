...
<div class="container">        
      <div class="row justify-content-end" style="background:#fff; border: dashed seagreen; border-radius: 15px 50px;">
        <div class="">
        <a href="{{ url('/kitnews/news/') }}"><button class="btn btn-colorx2  ml-2 mt-2 mb-2 mr-5" >اخبار منوعة</button></a> 
...
@if(!isset($_GET['page']))
    <li class="page-item disabled">
      <a class="page-link" href="{{url('kitnews/news/id/?page=')}}" tabindex="-1">الصفحة السابقة</a>
    </li>
    @for($i=0;$i<$numpages;$i++)
    @if($i==0)
    <li class="page-item active"><a class="page-link" href="{{url('kitnews/news?page='.$i)}}">{{$i}}</a></li>
    @else
    <li class="page-item "><a class="page-link" href="{{url('kitnews/news?page='.$i)}}">{{$i}}</a></li>
    @endif
    @endfor
    <li class="page-item">
      <a class="page-link" href="{{url('kitnews/news?page=1')}}">الصفحة التالية</a>
    </li>


    @elseif($_GET['page'] == $numpages-1)
    <li class="page-item ">
      <a class="page-link" href="{{url('kitnews/news?page='.$_GET['page']-=1)}}" tabindex="-1">الصفحة السابقة</a>
    </li>
    @for($i=0;$i<$numpages;$i++)
    @if($i==$numpages-1)
    <li class="page-item active"><a class="page-link" href="{{url('kitnews/news?page='.$i)}}">{{$i}}</a></li>
    ...
    @endfor
    <li class="page-item disabled">
      <a class="page-link" href="{{url('kitnews/news?page='.$_GET['page']+=1)}}" >الصفحة التالية</a>
    </li>
...
@elseif($_GET['page']>0 && $_GET['page']<$numpages-1)
    ...
    @endfor
    <li class="page-item">
      <a class="page-link" href="{{url('kitnews/news?page='.$_GET['page']+=2)}}" >الصفحة التالية</a>
    </li>
