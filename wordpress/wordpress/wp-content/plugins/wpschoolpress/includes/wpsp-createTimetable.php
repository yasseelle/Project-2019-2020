...
if ('POST' == $_SERVER['REQUEST_METHOD'] && isset($_POST['noh']) && $_POST['noh'] != '') {
    ...
    if (!empty($check_tt)) {
        ...
        if (empty($sessions)) {
            echo "<div class='wpsp-text-red'>No Working Hours added. Please add at <a href='admin.php?page=sch-settings&sc=WrkHours'>Settings</a></div>";
            return false;
        }
?>
...
<div class="wpsp-card">
  <div class="wpsp-card-head">
    <h3 class="wpsp-card-title">Drag and Drop Subjects
    </h3>
  </div>
...
?>
    <div class="wpsp-row">
      <div class="wpsp-col-md-12">
      <div class="wpsp-form-group">
        <label class="wpsp-labelMain">         Class :
          <?php
...
?>
      ...
      foreach ($session as $key => $ses)  {
            ...
            if ($hour_det->type == "1") {
                ...
                $td_class = "break";
                 $sub_name='<div class="item1 assigned wpsp-assigned-item <?php echo $key; ?> <?php echo $ses; ?> <?php echo $j;?>">Break</div>';
            }
            $sub_id   = '';
            $sub_name = '';
...
?>
  <td class="<?php echo $td_class; ?>" tid="<?php echo $ses; ?>" data-sessionid="<?php echo $key; ?>" ><?php if($td_class == "break"){ echo "Break";} else {echo $sub_name; }?></td>
            <?php
        }
?>
          </tr>
...
?>
    <div class="wpsp-card">
      <div class="wpsp-card-head">
        <h3 class="wpsp-card-title"><?php echo apply_filters( 'wpsp_timetable_heading_item',esc_html("Add New Timetable","WPSchoolPress")); ?>
        </h3>
      </div>
      <div class="wpsp-card-body">
      <form class="wpsp-form-horizontal" id="timetable_form" action="" method="post">
        ...
        <div class="wpsp-col-lg-3 wpsp-col-md-4 wpsp-col-sm-4 wpsp-col-xs-12">
          <div class="wpsp-form-group">
            <label class="wpsp-label"><?php
              $pl = "";
              if(isset($item['wpsp_class_name'])){
                    echo $pl = esc_html($item['wpsp_class_name'],"WPSchoolPress");
              }else{
                  echo $pl = esc_html("Select Class","WPSchoolPress");
              }
              /*Check Required Field*/
              if(isset($is_required_item['wpsp_class_name'])){
                  $is_required =  $is_required_item['wpsp_class_name'];
              }else{
                  $is_required = true;
              }
              ?>
            ...
            <select name="wpsp_class_name" data-is_required="<?php echo $is_required; ?>" id="wpsp_class_name" class="wpsp-form-control">
        ...
        <div class="wpsp-col-lg-3 wpsp-col-md-4 wpsp-col-sm-4 wpsp-col-xs-12">
          <div class="wpsp-form-group">
            <label class="wpsp-label"><?php
              $pl = "";
              if(isset($item['sessions_template'])){
                    echo $pl = esc_html($item['sessions_template'],"WPSchoolPress");
              }else{
                  echo $pl = esc_html("Choose Session Template","WPSchoolPress");
              }
              /*Check Required Field*/
              if(isset($is_required_item['sessions_template'])){
                  $is_required =  $is_required_item['sessions_template'];
              }else{
                  $is_required = true;
              }
              ?>
        ...
        <div class="wpsp-col-lg-3 wpsp-col-md-4 wpsp-col-sm-4 wpsp-col-xs-12" id="enter_sessions">
          <div class="wpsp-form-group">
            <label class="wpsp-label"><?php
              $pl = "";
              if(isset($item['noh'])){
                    echo $pl = esc_html($item['noh'],"WPSchoolPress");
              }else{
                  echo $pl = esc_html("Enter No.of Sessions","WPSchoolPress");
              }
              /*Check Required Field*/
              if(isset($is_required_item['noh'])){
                  $is_required =  $is_required_item['noh'];
              }else{
                  $is_required = true;
              }
              ?>
            ...
            <input type="text" data-is_required="<?php echo $is_required; ?>" name="noh" class="wpsp-form-control">
